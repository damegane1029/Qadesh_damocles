Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/846e5ee3508707f4a8e4257d78d317c9"
{
	Properties
	{
...
#define PROP_FLIPBOOKTOTALFRAMES 210
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				float4 flipbookScaleOffset = float4(1,0.73,0,0);
				...
				if (!float(0))
				{
					if (float(15) != 0)
					{
						currentFrame = (_Time.y / (1 / float(15))) % float(210);
					}
				}
				else
				{
					currentFrame = fmod(float(0), float(210));
				}
				...
				if (float(0))
				{
					float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(210))));
