Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/3a45b140ef234404ba2e9d8e05b44242"
{
	Properties
	{
...
#define PROP_FLIPBOOKTOTALFRAMES 41
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					if (float(10) != 0)
					{
						currentFrame = (_Time.y / (1 / float(10))) % float(41);
					}
				}
				else
				{
					currentFrame = fmod(float(0), float(41));
				}
				...
				if (float(0))
				{
					float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(41))));
