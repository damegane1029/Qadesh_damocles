Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/013337a2ef2d26042aa489e254d54c49"
{
	Properties
	{
...
#define PROP_FLIPBOOKTOTALFRAMES 50
#define PROP_FLIPBOOKFPS 20
#define PROP_FLIPBOOKTILED 0
#define PROP_FLIPBOOKEMISSIONSTRENGTH 20
#define PROP_FLIPBOOKROTATION 180
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				float4 flipbookScaleOffset = float4(1.3,1.3,0,0);
				flipbookScaleOffset.xy = 1 - flipbookScaleOffset.xy;
				float2 uv = frac(poiMesh.uv[float(0)]);
				float theta = radians(float(180) + _Time.z * float(0));
				float cs = cos(theta);
				float sn = sin(theta);
				...
				float4 sideOffset = float4(-float4(0,0,-0.23,0).x, float4(0,0,-0.23,0).y, -float4(0,0,-0.23,0).z, float4(0,0,-0.23,0).w);
				...
				if (!float(0))
				{
					if (float(20) != 0)
					{
						currentFrame = (_Time.y / (1 / float(20))) % float(50);
					}
				}
				else
				{
					currentFrame = fmod(float(0), float(50));
				}
				...
				if (float(0))
				{
					float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(50))));
				...
				float flipbookEmissionStrength = float(20);
